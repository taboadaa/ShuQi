...
static void on_stuff_manager_evt(ble_stuff_manager_t * p_stuff_manager, ble_stuff_manager_evt_t * p_evt) {
	...
	case BLE_STUFF_MANAGER_ENTRY_SELECTION_EVT_WRITE:
		...
		ble_entry_selection = p_evt->params.entry_selection.entry_number;
	...
	case BLE_STUFF_MANAGER_MANAGER_MODE_EVT_WRITE:
		...
		manager_mode_mode_t tmp = p_evt->params.manager_mode.mode;
		if (tmp.mode == MODE_SLEEP) {
			ble_manager_mode = STATE_SLEEP;
		} else if (tmp.mode == MODE_READ) {
			ble_manager_mode = STATE_READ;
		} else if (tmp.mode == MODE_RECOGNITION) {
			ble_manager_mode = STATE_RECOGNITION;
		}
...
uint32_t bluetooth_init(void) {
	...
	memset(&stuff_manager_init.ble_stuff_manager_entry_selection_initial_value.entry_number, 0x00,
	...
	memset(&stuff_manager_init.ble_stuff_manager_entry_number_initial_value.entry_number, 0x00,
...
void ble_set_stuff_manager_entry_value(uint8_array_t value) {
	ble_stuff_manager_entry_value_t * p_entry_value = (ble_stuff_manager_entry_value_t *) malloc(sizeof(ble_stuff_manager_entry_value_t));
