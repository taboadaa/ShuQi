...
uint32_t add_tag_buffer_ble(uint8_t* buffer_ble, TagUHF_t *tag) {
	...
	for (uint8_t i = 0; i < 12; i++) {
		// le format de tag est stocker en MSB en premier mais est envoy� par BLE avec LSB en premier
...
uint32_t tag_rfid_to_format_ble(uint8_array_t* buffer_ble, Buffer_tag_UHF_t* buffer_tag_uhf) {
	...
	for (uint8_t i = 0; i < buffer_tag_uhf->size; i++) {
		...
		uint8_array_t newTag;
		newTag.size = RFID_ID_SIZE;
		newTag.p_data = buffer_tag_uhf->TagUHF[i].EPC;

		rfid_ids_add(buffer_ble, i, newTag);
		nbTag++;
	}
	return nbTag;
}

uint16_t rfid_read_tags(uint8_array_t* buffer_ble) {
	int pouet = 0;
	pouet++;
	...
	uint16_t nbTag = tag_rfid_to_format_ble(buffer_ble, &Buffer_tag_UHF);
